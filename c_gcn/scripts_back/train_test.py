...
init_kwargs = {
    ...
    'dataset_cls': 'graph_vqa2_new_dataset',

    'criterion_cls': 'vqa2_new_cross_entropy',
    'seed': 1,
    'verbose': False,
    ...
    'graph_vqa_net_layer_methods': ('linear', 'max^film_softmax^4^12_linear_film^sum',
...
def epoch_eval_fn(trainer,
    ...
    checkpoint = checkpoint or trainer.checkpoint
    logger_group: pt.LoggerGroup = logger_group or trainer.logger_group

    a_vocab = loader.loader.dataset.answer_vocab
    ret_json = list()
    for step_idx in pbar:
        ...
        with torch.no_grad():
            ...
            if isinstance(model_input, dict):
                ...
                model_output = model(*model_input)
            log = criterion.test(model_output, sample)
    ...
    json.dump(ret_json, checkpoint.checkpoint_dir.joinpath(f'test_{epoch_idx}.json').open('w'))


trainer = pt.Trainer.build(**init_kwargs)
trainer(epoch_eval_fn=epoch_eval_fn)
